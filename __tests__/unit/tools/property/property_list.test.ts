...
import { TestData, assertSuccessResponse } from '../../../../testing/test-helpers';
import { z } from 'zod';
...
describe('Property - List', () => {
  ...
  const testConfig: ToolTestConfig = {
    ...
    handler: propertyOperations.property_list?.handler || propertyOperations['property_list']?.handler,
    ...
    responseValidator: (response: any) => {
  ...
  testConfig.testCases.forEach((testCase: any) => {
  ...
  it('should complete within performance threshold', async () => {
    ...
    const results = await runner.runTests({
      ...testConfig,
      testCases: testConfig.testCases.length > 0 ? [testConfig.testCases[0]].filter(Boolean) : [] // Just run one test if available
    });
