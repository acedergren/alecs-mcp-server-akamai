...
import { OrchestrationAkamaiOperations } from '../../tools/orchestration/workflows';
...
jest.mock('../../core/server/performance/smart-cache', () => ({
  getGlobalCache: jest.fn(() => ({ initialize: jest.fn().mockResolvedValue(undefined), get: jest.fn(), set: jest.fn(), delete: jest.fn(), clear: jest.fn() }))
}));
