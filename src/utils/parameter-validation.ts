...
export const AkamaiIdSchemas = {
  ...
  activationId: z.string().regex(AKAMAI_ID_PATTERNS.activation, 'Activation ID must match pattern act_*'),
  cpCodeId: z.string().regex(AKAMAI_ID_PATTERNS.cpCode, 'CP Code ID must match pattern cpc_*'),
  edgeHostnameId: z.string().regex(AKAMAI_ID_PATTERNS.edgeHostname, 'Edge Hostname ID must match pattern ehn_*'),
  ...
  networkListId: z.string().regex(AKAMAI_ID_PATTERNS.networkList, 'Network List ID must be alphanumeric with underscores'),
...
export const NetworkSchemas = {
  ...
  ipv6: z.string()
    .regex(/^(?:[0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}$|^::$/, 'Invalid IPv6 address'),
  
  cidr: z.string()
    .regex(/^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\/(?:[0-9]|[1-2][0-9]|3[0-2])$/, 'Invalid CIDR notation'),
...
export const AkamaiEnums = {
  network: z.enum(['STAGING', 'PRODUCTION'], { errorMap: () => ({ message: 'Network must be STAGING or PRODUCTION' }) }),
...
export const PropertyManagerSchemas = {
  ...
  activateProperty: z.object({
    ...
    notifyEmails: z.array(NetworkSchemas.email)
    ...
    (data) => data.network === 'STAGING' || (data.network === 'PRODUCTION' && data.notifyEmails && data.notifyEmails.length > 0),
    {
...
export const DNSSchemas = {
  ...
  createZone: z.object({
    ...
    (data) => data.type !== 'SECONDARY' || (data.masters && data.masters.length > 0),
    {
      message: 'Secondary zones require master servers',
      path: ['masters']
    }
  ).refine(
    (data) => data.type !== 'ALIAS' || data.target,
    {
  ...
  upsertRecord: z.object({
    ...
    rdata: z.array(z.string().min(1, 'Record data cannot be empty'))
...
export const CertificateSchemas = {
  createDVEnrollment: z.object({
    ...
    networkConfiguration: z.object({
      ...
      secureNetwork: z.enum(['standard-tls', 'enhanced-tls', 'shared-cert']).optional().default('enhanced-tls'),
...
export const FastPurgeSchemas = {
  purgeByUrl: z.object({
    customer: z.string().optional(),
    urls: z.array(NetworkSchemas.url)
  ...
  purgeByCpcode: z.object({
    customer: z.string().optional(),
    cpcodes: z.array(z.number().int().positive())
