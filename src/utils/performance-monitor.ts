...
export class PerformanceMonitor {
  ...
  analyzePerformance(operationType?: string, timeWindowMs?: number): PerformanceAnalysis {
    ...
    const timeSpanMs = timeWindowMs || (metrics.length > 0 ? Date.now() - Math.min(...metrics.map(m => m.startTime)) : 1000);
  ...
  private generateRecommendations(analysis: Partial<PerformanceAnalysis>): string[] {
    ...
    if (analysis.averageResponseTime! > this.thresholds.slowOperationMs) {
      recommendations.push(`Average response time (${analysis.averageResponseTime!.toFixed(0)}ms) exceeds threshold. Consider enabling caching or optimizing API calls.`);
    }
    
    if (analysis.p95ResponseTime! > this.thresholds.slowOperationMs * 2) {
      recommendations.push(`P95 response time (${analysis.p95ResponseTime!.toFixed(0)}ms) is very high. Investigate slowest operations and consider request batching.`);
    }
    
    if (analysis.errorRate! > this.thresholds.highErrorRatePercent) {
      recommendations.push(`Error rate (${analysis.errorRate!.toFixed(1)}%) is high. Check circuit breaker status and API health.`);
    }
    
    if (analysis.cacheHitRate! < this.thresholds.lowCacheHitRatePercent) {
      recommendations.push(`Cache hit rate (${analysis.cacheHitRate!.toFixed(1)}%) is low. Review cache strategy and TTL settings.`);
    }
    
    if (analysis.slowestOperations && analysis.slowestOperations.length > 0) {
      const slowestOp = analysis.slowestOperations[0];
      if (slowestOp && slowestOp.duration && slowestOp.duration > this.thresholds.slowOperationMs) {
        recommendations.push(`Slowest operation (${slowestOp.operationType}) took ${slowestOp.duration}ms. Consider optimization.`);
      }
    }
    ...
    if (memoryUsageMB > this.thresholds.highMemoryUsageMB) {
      recommendations.push(`Memory usage (${memoryUsageMB.toFixed(1)}MB) is high. Consider implementing memory-efficient caching.`);
    }
