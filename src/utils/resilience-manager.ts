...
import { ErrorTranslator } from './errors.js';
...
export class RetryHandler {
  ...
  async execute<T>(operation: () => Promise<T>, errorHandler?: (error: any, attempt: number) => boolean): Promise<T> {
    let lastError: any;
...
export class ErrorClassifier {
  private static categories: Map<string, ErrorCategory> = new Map([
    ...
    ['ENOTFOUND', {
      ...
      severity: ErrorSeverity.HIGH,
      recovery: [
        { type: 'RETRY', config: { maxAttempts: 2, baseDelay: 500 } }
      ]
    }],
    ['ETIMEDOUT', {
      type: 'NETWORK',
      ...
      recovery: [
        { type: 'RETRY', config: { maxAttempts: 3, baseDelay: 2000 } }
      ]
    }],
    ...
    ['404', {
      ...
      severity: ErrorSeverity.LOW,
      recovery: [
        { type: 'FALLBACK', config: { strategy: 'return_empty' } }
      ]
    }],
    ['429', {
      type: 'RATE_LIMIT',
    ...
    ['503', {
      ...
      severity: ErrorSeverity.HIGH,
      recovery: [
        { type: 'RETRY', config: { maxAttempts: 5, baseDelay: 3000 } }
      ]
    }],
    ['504', {
      type: 'SERVER_ERROR',
      ...
      severity: ErrorSeverity.MEDIUM,
      recovery: [
        { type: 'RETRY', config: { maxAttempts: 3, baseDelay: 5000 } }
      ]
    }]
  ]);
  ...
  static classify(error: any): ErrorCategory {
    ...
    return {
      ...
      severity: ErrorSeverity.MEDIUM,
      recovery: [
        { type: 'ALERT', config: { message: 'Unknown error occurred' } }
      ]
    };
  }
