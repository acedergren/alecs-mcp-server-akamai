...
import { TIMEOUTS, CACHE_TTL } from '../constants/index';
...
const coalescer = new RequestCoalescer({
  ...
  maxSize: 500,
  cleanupInterval: TIMEOUTS.EDGEGRID_DEFAULT,
});
...
class PropertyOperations {
  ...
  private transformPropertyList(response: any) {
    return {
      properties: (response as any).properties?.items || [],
      _meta: {
        total: (response as any).properties?.items?.length || 0,
        cached: false,
      },
    };
  }
  ...
  private transformProperty(response: any) {
    const property = (response as any).properties?.items?.[0] || response;
    return {
...
class ActivationOperations {
  ...
  private async pollActivationStatus(
    ...
    while (attempts < maxAttempts) {
      ...
      // Still pending
      await new Promise(resolve => setTimeout(resolve, TIMEOUTS.PROPERTY_ACTIVATION_DELAY));
      attempts++;
    }
