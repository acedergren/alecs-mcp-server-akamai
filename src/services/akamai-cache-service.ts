...
export interface PropertySearchResult {
  type: 'exact_hostname' | 'hostname_with_www' | 'hostname_without_www' | 'property_name' | 'property_id';
  property: any;
  hostname?: any;
  matchReason?: string;
}
...
export class AkamaiCacheService {
  ...
  private async createHostnameMapping(
    ...
    await this.cache.set(
      ...
      hostnameMap,
      CacheTTL.HOSTNAME_MAP
    );
    
    const elapsed = Date.now() - startTime;
    console.error(`[Cache] Created hostname mapping with ${Object.keys(hostnameMap).length} entries in ${elapsed}ms`);
  }
   ...
   */
  async getContracts(client: AkamaiClient, customer: string = 'default'): Promise<any[]> {
   ...
   */
  async getGroups(client: AkamaiClient, customer: string = 'default'): Promise<any[]> {
   ...
   */
  async warmCache(client: AkamaiClient, customer: string = 'default'): Promise<void> {
    ...
    try {
      ...
      await Promise.all([
        this.getContracts(client, customer),
        this.getGroups(client, customer),
      ]);
