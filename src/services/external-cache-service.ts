/**
 * External Cache Service for ALECS MCP Server
 * High-performance distributed caching with smart invalidation and monitoring
 * Supports Redis-compatible backends (Redis, Valkey, KeyDB, etc.)
 * 
 * @deprecated External cache support will be removed in v2.0.0.
 * Please migrate to the built-in SmartCache which provides excellent performance
 * for most use cases without external dependencies. If you require distributed
...
export class ExternalCache implements ICache {
  ...
  constructor(config: ExternalCacheConfig = {}) {
    console.warn(
      '\n[WARNING]  DEPRECATION WARNING: External cache support will be removed in v2.0.0.\n' +
    ...
    if (config.mode === 'cluster') {
      ...
      this.client = new Redis({
        host: config.nodes?.[0]?.host || process.env.CACHE_HOST || 'localhost',
        port: config.nodes?.[0]?.port || parseInt(process.env.CACHE_PORT || '6379'),
        password: config.password || process.env.CACHE_PASSWORD,
        db: config.db || 0,
        enableOfflineQueue: config.enableOfflineQueue !== false,
        ...
        retryStrategy: (times) => {
          if (times > 3) {
            console.error('[Cache] Max retries reached, disabling external cache');
            return null;
          }
  ...
  private setupEventHandlers(): void {
    this.client.on('error', (err) => {
      console.error('[Cache] External cache error:', err.message);
      this.connected = false;
      this.metrics.errors++;
    });

    this.client.on('connect', () => {
      console.error('[Cache] Connected to external cache successfully');
      this.connected = true;
    });

    this.client.on('ready', () => {
      console.error('[Cache] External cache ready to accept commands');
      this.connected = true;
    });

    this.client.on('close', () => {
      console.error('[Cache] External cache connection closed');
      this.connected = false;
    });
  }
  ...
  async get<T = any>(key: string): Promise<T | null> {
    ...
    try {
      ...
      console.error(`[Cache] Error getting ${key}:`, _err);
      this.metrics.errors++;
      return null;
    }
  }
  ...
  async set<T = any>(key: string, value: T, ttl: number): Promise<boolean> {
    ...
    try {
      ...
      console.error(`[Cache] Error setting ${key}:`, _err);
      this.metrics.errors++;
      return false;
    }
  }
  ...
  async del(keys: string | string[]): Promise<number> {
    ...
    try {
      ...
      console.error('[Cache] Error deleting keys:', _err);
      return 0;
    }
  }
  ...
  async ttl(key: string): Promise<number> {
    ...
    try {
      ...
      console.error(`[Cache] Error getting TTL for ${key}:`, _err);
      return -1;
    }
  }
  ...
  private async refreshInBackground<T>(
    ...
    try {
      ...
      console.error(`[Cache] Background refresh failed for ${key}:`, _err);
    } finally {
  ...
  async mget<T = any>(keys: string[]): Promise<Map<string, T>> {
    ...
    try {
      ...
      values.forEach((value, index) => {
        if (value) {
          try {
            ...
            console.error(`[Cache] Error parsing value for ${keys[index]}`);
          }
        } else {
          this.metrics.misses++;
        }
      ...
      console.error('[Cache] Error in mget:', _err);
      return new Map();
    }
  }
        ...
        stream.on('data', async (keys: string[]) => {
          if (keys.length) {
            try {
              ...
              console.error('[Cache] Error deleting batch:', _err);
            }
          }
        });
  ...
  async flushAll(): Promise<void> {
    ...
    try {
      ...
      console.error('[Cache] Error flushing cache:', _err);
    }
  }
