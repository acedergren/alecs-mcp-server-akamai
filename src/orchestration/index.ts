...
import { CDNProvisioningAgent, createCDNProvisioningAgent } from '../agents/cdn-provisioning.agent.js';
import { CPSCertificateAgent, createCPSCertificateAgent } from '../agents/cps-certificate.agent.js';
import { DNSMigrationAgent, createDNSMigrationAgent } from '../agents/dns-migration.agent.js';
import { format, icons, ProgressBar, MultiProgress } from '../utils/progress.js';
...
export class AkamaiOrchestrator {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.cpsAgent = await createCPSCertificateAgent(
        this.options.customer,
        this.dnsAgent
      );
  ...
  async migrateWebsite(options: {
    ...
    try {
      ...
      await this.dnsAgent!.migrateZoneComplete(
        options.domain,
        options.domain,
        {
      ...
      console.log(`${icons.success} Property activated to ${options.activateStaging ? 'staging' : 'ready for activation'}`);
  ...
  async provisionSecureWebsite(options: {
    ...
    console.log(`${icons.lock} Certificate: ${format.green(options.certificateType || 'default-dv')}`);
    console.log(`${icons.shield} Security: WAF ${options.enableWAF ? '✓' : '✗'} | DDoS ${options.enableDDoS ? '✓' : '✗'}`);
    console.log(format.dim('═'.repeat(60)));
    ...
    try {
      ...
      await this.cpsAgent!.provisionAndDeployCertificate(
        options.domains,
        {
        ...
        await Promise.all(
          batch.map(async (zone) => {
            ...
            try {
              ...
              results.failed++;
              results.errors.push(`${zone.source}: ${error.message}`);
            }
          })
        );
      }
