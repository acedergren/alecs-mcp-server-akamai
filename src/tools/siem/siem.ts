...
function formatSIEMEvents(response: z.infer<typeof SIEMResponseSchema>, configIds: string[]): string {
  ...
  text += `Events Retrieved: ${(response as any).events.length}\n`;
  ...
  (response as any).events.forEach((event: any) => {
  ...
  // Show sample events
  if ((response as any).events.length > 0) {
    text += `\n**Sample Events** (first 5):\n`;
    (response as any).events.slice(0, 5).forEach((event: any, index: any) => {
