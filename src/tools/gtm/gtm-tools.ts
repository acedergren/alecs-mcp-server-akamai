...
function formatDatacentersList(response: GTMResponse, domainName: string): string {
  const datacenters = response.datacenters || [];
...
function formatPropertyDetails(property: any, domainName: string): string {
  ...
  text += `**Type**: ${formatPropertyType(property.type)}\n`;
  text += `**Score Aggregation**: ${property.scoreAggregationType}\n`;
  text += `**Handout Mode**: ${property.handoutMode}\n`;
  
  if (property.trafficTargets && property.trafficTargets.length > 0) {
    text += `\n**Traffic Distribution**:\n`;
    text += formatTrafficDistribution(property.trafficTargets);
  }
  
  if (property.livenessTests && property.livenessTests.length > 0) {
    text += `\n**Liveness Tests**:\n`;
    property.livenessTests.forEach((test: any, index: number) => {
    ...
    async (client) => {
      return client.request({
        ...
        body: {
          ...
          loadImbalancePercentage: args.loadImbalancePercentage
        }
      });
    },
    {
...
export async function listDatacenters(args: z.infer<typeof GTMToolSchemas.listDatacenters>): Promise<MCPToolResponse> {
  return BaseTool.execute(
    ...
    {
      format: 'text',
      formatter: (result) => formatDatacentersList(result, args.domainName),
...
export async function listProperties(args: z.infer<typeof GTMToolSchemas.listProperties>): Promise<MCPToolResponse> {
  return BaseTool.execute(
    ...
    {
      format: 'text',
      formatter: (result) => formatPropertiesList(result, args.domainName),
 ...
 */
export async function getProperty(args: z.infer<typeof GTMToolSchemas.getProperty>): Promise<MCPToolResponse> {
  return BaseTool.execute(
    'gtm',
    'gtm_get_property',
    args,
...
export async function createProperty(args: z.infer<typeof GTMToolSchemas.createProperty>): Promise<MCPToolResponse> {
  return BaseTool.execute(
    ...
    async (client) => {
      return client.request({
        method: 'POST',
        path: GTMEndpoints.createProperty(args.domainName),
        body: {
 ...
 */
export async function updatePropertyTraffic(args: z.infer<typeof GTMToolSchemas.updatePropertyTraffic>): Promise<MCPToolResponse> {
  return BaseTool.execute(
    'gtm',
    ...
    async (client) => {
      ...
      property.trafficTargets = args.trafficTargets;
