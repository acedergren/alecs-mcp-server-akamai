...
export interface HostnameRoutingMapping {
  /** JSON path to hostname field (supports wildcards) */
  path: string;
  /** Whether to include context (property, cpcode, etc.) */
  includeContext?: boolean;
  /** Whether routing is enabled for this mapping */
  enabled?: boolean;
}
...
export abstract class AkamaiOperation {
   ...
   */
  static readonly COMMON_HOSTNAME_PATTERNS: Record<string, HostnameRoutingMapping[]> = {
    property: [
    ...
    all: [
      // Combine all common hostname patterns
      { path: '**.hostname', includeContext: true },
      { path: '**.edgeHostname', includeContext: true },
      { path: '**.name', includeContext: true },
      { path: '**.domains.*', includeContext: true },
