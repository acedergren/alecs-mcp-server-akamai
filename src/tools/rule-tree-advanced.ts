...
import { 
  ...
  type Behavior, 
  type Criterion,
  type PropertyVersionRulesGetResponse,
  type RuleError as PapiRuleError,
  type RuleWarning as PapiRuleWarning
...
export async function validateRuleTree(
  ...
  try {
    ...
    if (!rules && args.propertyId) {
      ...
      const typedPropertyResponse = propertyResponse as PropertyListResponse;
      ...
      const typedRulesResponse = rulesResponse as PropertyVersionRulesGetResponse;
      rules = typedRulesResponse.rules;
    }
...
export async function analyzeRuleTreePerformance(
  ...
  try {
    ...
    if (analysis.criticalFindings.length > 0) {
      ...
      analysis.criticalFindings.forEach((finding: AnalysisFinding, idx: number) => {
    ...
    if (analysis.bottlenecks.length > 0) {
      ...
      analysis.bottlenecks.forEach((bottleneck: Bottleneck, idx: number) => {
...
function validateRuleStructure(rule: RuleTree, validation: RuleValidationResult, path: string) {
...
function generateOptimizationSuggestions(rules: unknown): RuleOptimization[] {
  ...
  function analyzeRule(rule: RuleTree, path: string) {
...
interface RuleTreeAnalysis {
  ...
  caching: {
    ...
    dynamicStrategy: string;
    ttlOptimization: string;
    cacheKeyEfficiency: string;
  };
...
function performRuleTreeAnalysis(rules: RuleTree): RuleTreeAnalysis {
  const analysis = {
    complexity: 0,
    cacheEfficiency: 0,
    redundancy: 0,
