...
export class TodoGenerator {
     ...
     */
    public generateTodoList(analysisResults: AnalysisResults, options: GenerateOptions = {}): TodoList {
     ...
     */
    private generateRecommendationBasedTodos(recommendations: Recommendations, todos: TodoList): void {
    ...
    private processTodos(todos: TodoList, options: GenerateOptions): void {
        ...
        todos.items.sort((a, b) => {
            const priorityDiff = this.priorityMatrix[b.priority].weight - this.priorityMatrix[a.priority].weight;
        ...
        todos.metadata.estimatedTotalHours = todos.items.reduce((sum, item) => 
            sum + this.effortEstimates[item.effort].hours, 0);
        todos.metadata.criticalCount = todos.items.filter(item => item.priority === 'CRITICAL').length;
    ...
    private hasDependency(prerequisite: TodoItem, dependent: TodoItem): boolean {
        ...
        return dependencyRules.some(rule => 
            prerequisite.type === rule.prereq && rule.dep.includes(dependent.type)
        );
    }
    ...
    private identifyQuickWins(todos: TodoList): void {
        todos.quickWins = todos.items.filter(item => {
    ...
    private generateLongTermGoals(todos: TodoList): void {
        const longTermItems = todos.items.filter(item => 
            item.effort === 'major' || item.tags?.includes('long-term')
        );
     ...
     */
    public exportTodos(todos: TodoList, format: string = 'json'): string {
