...
export class FixStrategyOptimizer {
    ...
    generateStrategyOverview(analysisResults: AnalysisResults, todoList: TodoList) {
        ...
        const criticalItems = todoList.items?.filter(item => item.priority === 'CRITICAL').length || 0;
    ...
    optimizeQuickWins(candidates: TodoItem[]): TodoItem[] {
        ...
        scored
            ...
            .forEach(item => {
                if (selected.length < maxQuickWins && 
    ...
    createTimelinePhases(items: TodoItem[]): Phase[] {
        ...
        phases.forEach(phase => {
            const totalEffort = phase.items.reduce((sum, item) => 
                sum + (item.effort_details?.hours || 0), 0);
    ...
    recommendApproachFull(todoList: TodoList, analysisResults: AnalysisResults) {
        const criticalCount = todoList.items?.filter(item => item.priority === 'CRITICAL').length || 0;
    ...
    categorizeItem(item: TodoItem): string {
        ...
        if (tags.includes('infrastructure') || title.includes('infrastructure')) return 'infrastructure';
    ...
    calculateQuickWinImpact(quickWins: TodoItem[]) {
        const totalImpact = quickWins.reduce((sum, item) => 
            sum + (item.priority_details?.weight || 0), 0);
        const avgImpact = quickWins.length > 0 ? totalImpact / quickWins.length : 0;
        
        return {
            total: totalImpact,
            average: avgImpact,
            customerImpact: quickWins.filter(item => 
                item.tags?.includes('customer-facing')).length,
    ...
    private calculateSuccessProbability(todoList: TodoList, analysisResults: AnalysisResults): number {
    ...
    private calculateInitiativePriority(items: TodoItem[]): number {
        const avgPriority = items.reduce((sum, item) => 
            sum + (item.priority_details?.weight || 0), 0) / items.length;
        return avgPriority;
    }

    private assessInitiativeComplexity(items: TodoItem[]): string {
        const avgEffort = items.reduce((sum, item) => 
            sum + (item.effort_details?.hours || 0), 0) / items.length;
