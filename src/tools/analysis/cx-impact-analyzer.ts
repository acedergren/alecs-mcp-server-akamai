...
export class CustomerExperienceImpactAnalyzer {
    ...
    constructor() {
        this.customerPersonas = {
            'developer': {
                ...
                journeySteps: ['authentication', 'api_discovery', 'implementation', 'testing', 'deployment'],
                businessValue: 0.8,
            ...
            'security_admin': {
                ...
                journeySteps: ['security_setup', 'certificate_management', 'policy_configuration', 'compliance_monitoring'],
                businessValue: 0.95,
            ...
            'content_manager': {
                ...
                journeySteps: ['content_upload', 'cache_configuration', 'performance_monitoring', 'content_optimization'],
                businessValue: 0.7,
            ...
            'site_admin': {
                ...
                priority: 'HIGH',
                painPoints: ['site_availability_issues', 'performance_problems', 'configuration_complexity'],
    ...
    getPersonaMultiplier(failure: TestFailure): number {
        ...
        if (message.includes('security') || message.includes('certificate') || testName.includes('security')) {
            return 1.5;
        }
        ...
        if (message.includes('config') || message.includes('deployment') || testName.includes('config')) {
            return 1.3;
        }
     ...
     */
    analyzePersonaImpacts(testResults: TestResults, analysisResults: any): Record<string, PersonaImpact> {
     ...
     */
    calculatePersonaImpact(personaId: string, persona: CustomerPersona, testResults: TestResults, analysisResults: any): PersonaImpact {
            ...
            return persona.painPoints.some(painPoint => {
                switch (painPoint) {
                    ...
                    case 'deployment_failures':
                        return message.includes('deploy') || testName.includes('deploy') || testName.includes('activation');
                    case 'configuration_issues':
                        return message.includes('config') || testName.includes('config') || testName.includes('setup');
                    case 'monitoring_gaps':
                        return testName.includes('monitor') || testName.includes('health');
                    case 'security_misconfigurations':
                        return message.includes('security') || testName.includes('security') || message.includes('certificate');
                    ...
                    case 'access_control_failures':
                        return message.includes('unauthorized') || message.includes('forbidden') || message.includes('access');
                    ...
                    case 'performance_degradation':
                        return message.includes('timeout') || message.includes('slow') || testName.includes('performance');
                    case 'site_availability_issues':
                        return message.includes('unavailable') || message.includes('down') || testName.includes('availability');
                    case 'performance_problems':
                        return message.includes('performance') || message.includes('slow') || message.includes('timeout');
                    case 'configuration_complexity':
                        return message.includes('complex') || testName.includes('advanced') || message.includes('validation');
                    default:
                        return false;
                }
            });
     ...
     */
    analyzeJourneyImpacts(testResults: TestResults, analysisResults: any): Record<string, JourneyImpact> {
     ...
     */
    calculateJourneyImpact(journeyId: string, journey: CustomerJourney, testResults: TestResults, analysisResults: any): JourneyImpact {
    ...
    getJourneyRelevantFailures(journeyId: string, testResults: TestResults): TestFailure[] {
        ...
        return failures.filter(failure => {
            ...
            return stepNames.some(stepName => {
                return testName.includes(stepName.replace('_', '')) || 
     ...
     */
    matchesJourneyContext(journeyId: string, stepName: string, message: string, testName: string, suite: string): boolean {
        ...
        if (!journeyMappings || !journeyMappings[stepName]) return false;
     ...
     */
    calculateMetricImpact(metricName: string, metricConfig: BusinessMetric, impactMultiplier: number, testResults: TestResults, analysisResults: any): number {
    ...
    calculateApiErrorRate(testResults: TestResults): number {
        const apiTests = this.countFailuresByType(testResults, 'api') + this.countTestsByType(testResults, 'api');
        const apiFailures = this.countFailuresByType(testResults, 'api');
    ...
    identifyCustomerFacingFailures(testResults: TestResults): TestFailure[] {
        ...
        return failures.filter(failure => {
            ...
            return testName.includes('customer') ||
    ...
    estimateAffectedCustomers(testResults: TestResults, analysisResults: any, specificFailure?: TestFailure): string {
    ...
    getFailureAffectedPersonas(failure: TestFailure): string[] {
        ...
        if (message.includes('content') || message.includes('performance')) personas.push('content_manager');
    ...
    getFailureAffectedJourneys(failure: TestFailure): string[] {
        ...
        if (testName.includes('daily') || testName.includes('monitor')) journeys.push('daily_operations');
        if (testName.includes('incident') || testName.includes('critical')) journeys.push('incident_response');
        if (testName.includes('feature') || testName.includes('new')) journeys.push('feature_adoption');
        if (testName.includes('scale') || testName.includes('performance')) journeys.push('scaling_operations');
    ...
    private getAffectedJourneyStepsForJourney(journeyId: string, failures: TestFailure[]): JourneyStep[] {
        const journey = this.customerJourneys[journeyId];
        if (!journey) return [];
    ...
    private getOverallImpactMultiplier(testResults: TestResults, analysisResults: any): number {
        const failureRate = (testResults.summary?.failedTests || 0) / (testResults.summary?.totalTests || 1);
    ...
    private calculateOverallRiskScore(risks: { immediate: Risk[]; shortTerm: Risk[]; longTerm: Risk[] }): number {
        let score = 0;
