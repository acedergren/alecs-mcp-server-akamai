...
export class TestOutputAnalyzer {
    ...
    constructor() {
        this.errorPatterns = new Map<string, RegExp>([
            ...
            ['NETWORK_ERROR', /(?:ECONNREFUSED|ENOTFOUND|TIMEOUT|Connection.*refused|Network.*unreachable)/i],
            ['AUTH_ERROR', /(?:401|403|Unauthorized|Forbidden|Authentication.*failed|Invalid.*credentials)/i],
            ['RATE_LIMIT', /(?:429|Too.*many.*requests|Rate.*limit|Throttled)/i],
            
            // API/Service Errors
            ['API_ERROR', /(?:500|502|503|504|Internal.*server.*error|Bad.*gateway|Service.*unavailable)/i],
            ['VALIDATION_ERROR', /(?:400|Bad.*request|Validation.*failed|Invalid.*parameter|Missing.*required)/i],
            ['NOT_FOUND', /(?:404|Not.*found|Resource.*not.*found|Property.*not.*found)/i],
            
            // Configuration Errors
            ['CONFIG_ERROR', /(?:Configuration.*error|Missing.*config|Invalid.*config|\.edgerc.*not.*found)/i],
            ...
            ['PROPERTY_ERROR', /(?:Property.*validation.*failed|Rule.*tree.*error|Edge.*hostname.*error)/i],
     ...
     */
    public parseTestOutput(output: string, format: string = 'jest'): ParsedTestResults | { success: false; error: string; results: [] } {
        try {
    ...
    private parseJestJsonOutput(jsonOutput: JestJsonOutput): ParsedTestResults {
        const results: ParsedTestResults = {
            summary: {
                ...
                runtime: jsonOutput.testResults?.reduce((sum, suite) => sum + (suite.perfStats?.runtime || 0), 0) || 0
            },
            testSuites: [],
            failures: [],
            errors: [],
    ...
    private generateOverview(testResults: ParsedTestResults): Overview {
        ...
        if (testResults.testSuites) {
            overview.totalRuntime = testResults.testSuites.reduce((sum, suite) => sum + (suite.runtime || 0), 0);
            overview.averageTestTime = overview.totalTests > 0 ? overview.totalRuntime / overview.totalTests : 0;
        }
     ...
     */
    private assessTestHealth(overview: Overview): 'EXCELLENT' | 'GOOD' | 'FAIR' | 'POOR' | 'CRITICAL' {
    ...
    private analyzeErrors(testResults: ParsedTestResults): ErrorAnalysis {
        ...
        allErrors.forEach(error => {
            ...
            errorAnalysis.errorFrequency.set(category, (errorAnalysis.errorFrequency.get(category) || 0) + 1);

            // Update severity distribution
            errorAnalysis.severityDistribution.set(severity, (errorAnalysis.severityDistribution.get(severity) || 0) + 1);
     ...
     */
    private assessErrorSeverity(error: any, category: string): 'CRITICAL' | 'HIGH' | 'MEDIUM' | 'LOW' {
    ...
    private findCascadingFailures(testResults: ParsedTestResults): CascadingFailure[] {
        ...
        const setupFailures = (testResults.failures || []).filter(f => 
    ...
    private findTimeBasedPatterns(testResults: ParsedTestResults): TimeBasedPattern {
        ...
        return {
            ...
            performanceIssues: overview.averageTestTime > 5000 ? ['Tests are running slower than expected'] : []
        };
    }
    ...
    private assessImpact(testResults: ParsedTestResults): ImpactAssessment {
        ...
        const impacts = [impact.businessImpact.level, impact.technicalImpact.level, impact.customerImpact.level];
    ...
    private assessBusinessImpact(testResults: ParsedTestResults): BusinessImpact {
        ...
        const criticalFailures = (testResults.failures || []).filter(f => 
            f.message.includes('production') || f.message.includes('critical') || f.message.includes('security')
        ).length;
    ...
    private assessCustomerImpact(testResults: ParsedTestResults): CustomerImpact {
        const customerFacingTests = (testResults.testSuites || []).filter(suite => 
        ...
        const failedCustomerTests = customerFacingTests.filter(suite => suite.status === 'fail').length;
        ...
        const securityFailures = (testResults.failures || []).filter(f =>
            f.message.includes('auth') || f.message.includes('security') || f.message.includes('unauthorized')
        );
