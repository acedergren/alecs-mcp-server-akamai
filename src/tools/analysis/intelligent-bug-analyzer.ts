...
export class IntelligentBugAnalyzer {
    ...
    private generateExecutiveSummary(data: {
        ...
        const criticalIssues = todoList.items?.filter(item => item.priority === 'CRITICAL').length || 0;
     ...
     */
    private generateCustomerImpactSummary(customerImpact: CustomerImpact | null): CustomerImpactSummary {
        if (!customerImpact) {
            return {
                level: 'UNKNOWN',
    ...
    private calculateHealthScore(testResults: TestResults, testAnalysis: TestAnalysis): HealthScore {
        ...
        const criticalFailures = testAnalysis.errorAnalysis?.categorizedErrors?.get('AUTH_ERROR')?.length || 0;
    ...
    private calculatePerformanceScore(testResults: TestResults): number {
        const avgRuntime = (testResults.testSuites?.reduce((sum, suite) => 
            sum + (suite.runtime || 0), 0) || 0) / (testResults.testSuites?.length || 1);
