...
function getCircuitBreakerEmoji(state: CircuitBreakerState): string {
  switch (state) {
    case CircuitBreakerState.CLOSED: return '🟢';
    case CircuitBreakerState.HALF_OPEN: return '🟡';
    case CircuitBreakerState.OPEN: return '🔴';
    default: return '⚪';
  }
}
...
async function performTestOperation(client: AkamaiClient, operationType: OperationType): Promise<void> {
  // Simple test operations that don't modify data
  switch (operationType) {
    case OperationType.PROPERTY_READ:
      await globalResilienceManager.executeWithResilience(
        operationType,
