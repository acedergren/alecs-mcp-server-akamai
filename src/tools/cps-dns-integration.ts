  ...
  try {
    ...
    return {
      content: [
        {
          type: 'text',
          text: `❌ Failed to create ACME validation records: ${_error instanceof Error ? _error.message : 'Unknown error'}`,
        },
      ],
    };
  }
...
export async function monitorCertificateValidation(
  ...
  while (Date.now() - startTime < maxWait) {
    ...
    try {
      ...
      spinner.stop();
      console._error(
        `\n❌ Error checking validation status: ${_error instanceof Error ? _error.message : 'Unknown error'}`,
      );
      await new Promise((resolve) => setTimeout(resolve, checkInterval));
    }
  }
