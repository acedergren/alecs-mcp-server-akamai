...
export const DiagnosticsEndpoints = {
  ...
  asyncRequestStatus: (requestId: string) => `${DIAGNOSTICS_API_BASE}/requests/${requestId}`,
  testUrl: () => `${DIAGNOSTICS_API_BASE}/test-url`
};
...
export const DiagnosticsToolSchemas = {
  ...
  runGrep: GrepRequestSchema.extend({
    customer: z.string().optional().describe('Akamai account name/ID (defaults to "default")')
  }),

  getAsyncRequestStatus: z.object({
    requestId: z.string().describe('Request ID from async operation'),
    customer: z.string().optional().describe('Akamai account name/ID (defaults to "default")')
  }),
