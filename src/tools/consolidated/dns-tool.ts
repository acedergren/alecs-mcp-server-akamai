...
const DNSToolSchema = z.object({
  ...
  options: z
    .object({
      // Business shortcuts
      businessAction: z
        .enum(['setup-email', 'add-subdomain', 'enable-ssl', 'verify-ownership', 'setup-redirects'])
        .optional(),

      // Email provider shortcuts
      emailProvider: z.enum(['google-workspace', 'microsoft-365', 'custom']).optional(),
...
export const dnsTool: Tool = {
  name: 'dns',
  description:
    'Safe DNS management with business shortcuts. Manage zones, records, and configurations with built-in safety checks.',
  inputSchema: {
    type: 'object',
