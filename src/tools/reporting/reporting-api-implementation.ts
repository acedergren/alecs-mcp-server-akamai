...
export function formatTrafficReport(response: any, args: any): string {
  ...
  if (data.length === 0) {
    text += '\n⚠️ No traffic data found for the specified period.\n';
    return text;
  }
...
export function formatPerformanceReport(response: any, args: any): string {
  ...
  if (data.length === 0) {
    text += '\n⚠️ No performance data found for the specified period.\n';
    return text;
  }
  ...
  if (avgCacheHit < 80) {
    text += `\n⚠️ **Performance Alert:** Cache hit rate is below 80%. Consider optimizing cache settings.\n`;
  }
  
  const avgOrigin = avgMetrics.originTime / avgMetrics.count;
  if (avgOrigin > 1000) {
    text += `\n⚠️ **Performance Alert:** Origin response time exceeds 1 second. Check origin server performance.\n`;
  }
...
export function formatErrorReport(response: any, args: any): string {
  ...
  if (errors.length === 0) {
    text += '\n✅ No errors found for the specified period.\n';
    return text;
  }
  ...
  errors.forEach((error: any) => {
    ...
    byStatusCode[code] = (byStatusCode[code] || 0) + error.count;
  ...
  if (args.groupBy === 'url' && errors[0]?.url) {
    ...
    errors.slice(0, 5).forEach((error: any) => {
      text += `• ${error.url}: ${error.count.toLocaleString()} errors\n`;
    });
  }
...
export function formatGeographyReport(response: any): string {
  ...
  if (data.length === 0) {
    text += '⚠️ No geographic data found.\n';
    return text;
  }
