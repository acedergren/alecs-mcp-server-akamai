/**
 * Reporting Tools for ALECS MCP Server
 * 
 ...
 * - Uses functional pattern with AkamaiOperation.execute
 * 
 ...
 */
export async function getTrafficReport(args: z.infer<typeof TrafficReportSchema>): Promise<MCPToolResponse> {
  ...
  return AkamaiOperation.execute(
    ...
    async (client) => {
      ...
      const formattedData = validatedResponse.data.map((item) => ({
      ...
      return {
        ...
        data: formattedData,
        summary: {
          totalEdgeHits: validatedResponse.summary.total_edge_hits,
          totalEdgeBandwidth: validatedResponse.summary.total_edge_bandwidth,
          totalOriginHits: validatedResponse.summary.total_origin_hits,
          totalOriginBandwidth: validatedResponse.summary.total_origin_bandwidth,
          averageOffloadRate: validatedResponse.summary.avg_offload_rate
        }
      };
    },
    {
      cacheKey: `traffic:${params.start_date}:${params.end_date}:${params.granularity}:${params.metrics.join(',')}`,
 ...
 */
export async function getCachePerformance(args: z.infer<typeof CachePerformanceSchema>): Promise<MCPToolResponse> {
  ...
  return AkamaiOperation.execute(
    ...
    async (client) => {
      ...
      return {
        ...
        performance: validatedResponse.data.map((item) => ({
        ...
        summary: {
          overallHitRate: `${(validatedResponse.summary.overall_hit_rate * 100).toFixed(2)}%`,
          overallOffloadRate: `${(validatedResponse.summary.overall_offload_rate * 100).toFixed(2)}%`,
          totalCacheHits: validatedResponse.summary.total_cache_hits,
          totalRequests: validatedResponse.summary.total_requests
        }
      };
    },
    {
      cacheKey: `cache:${params.start_date}:${params.end_date}:${params.granularity}`,
      cacheTtl: 300 // 5 minutes
    }
  );
}
 ...
 */
export async function getGeographicDistribution(args: z.infer<typeof GeographicDistributionSchema>): Promise<MCPToolResponse> {
  ...
  return AkamaiOperation.execute(
    ...
    async (client) => {
      ...
      const sortedData = validatedResponse.data
      ...
      return {
        ...
        totalLocations: validatedResponse.total_locations
      };
    },
    {
      cacheKey: `geo:${params.start_date}:${params.end_date}:${params.level}:${params.metrics.join(',')}`,
      cacheTtl: 300 // 5 minutes
    }
  );
}
 ...
 */
export async function getErrorAnalysis(args: z.infer<typeof ErrorAnalysisSchema>): Promise<MCPToolResponse> {
  ...
  return AkamaiOperation.execute(
    ...
    async (client) => {
      ...
      return {
        ...
        errors: validatedResponse.data.map((error) => ({
        ...
        summary: {
          totalErrors: validatedResponse.summary.total_errors,
          errorRate: `${(validatedResponse.summary.error_rate * 100).toFixed(2)}%`,
          mostCommonErrors: validatedResponse.summary.most_common_errors
        }
      };
    },
    {
      cacheKey: `errors:${params.start_date}:${params.end_date}:${params.granularity}`,
      cacheTtl: 300 // 5 minutes
    }
  );
}
 ...
 */
export async function getRealTimeMetrics(args: {
  customer?: string;
 ...
 */
export async function getOriginPerformance(args: {
  customer?: string;
  start_date: string;
  end_date: string;
  origins?: string[];
 ...
 */
export async function getSecurityThreats(args: {
  customer?: string;
  start_date: string;
  end_date: string;
 ...
 */
export async function createCustomDashboard(args: {
  customer?: string;
...
/**
 * Consolidated reporting tools object for backwards compatibility
 */
export const consolidatedReportingOperations = {
  getTrafficReport,
  getCachePerformance,
...
// Export for registry.ts
export const consolidatedReportingAkamaiOperations = consolidatedReportingOperations;