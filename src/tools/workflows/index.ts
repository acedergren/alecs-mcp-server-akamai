...
import { dnsDomainAssistant, handleDNSDomainAssistant } from './dns-workflow-assistant';
 ...
 */
export async function handleWorkflowAssistantRequest(toolName: string, args: any): Promise<any> {
...
export function discoverAssistant(userIntent: string): {
  ...
  // Secondary scoring
  if (intent.includes('new') || intent.includes('create')) {
    scores.property += 1;
  }
  if (intent.includes('email')) {
    scores.dns += 1;
  }
  if (intent.includes('hack') || intent.includes('threat')) {
    scores.security += 1;
  }
  if (intent.includes('metric') || intent.includes('report')) {
 ...
 */
export async function handleCrossWorkflowRequest(intent: string, context: any): Promise<any> {
      ...
      content: [
        {
          type: 'text',
          text:
            `I see you need help with multiple areas. Let me connect you with the right assistants:\n\n` +
            `**Primary:** ${workflowAssistantMetadata[discovery.recommended as keyof typeof workflowAssistantMetadata].name}\n` +
            `**Also relevant:** ${discovery.alternates
              .map(
                (a) => workflowAssistantMetadata[a as keyof typeof workflowAssistantMetadata].name,
              )
              .join(', ')}\n\n` +
