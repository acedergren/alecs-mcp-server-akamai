...
export const propertyInfrastructureAssistant: Tool = {
  name: 'infrastructure',
  description:
    "Maya's intelligent infrastructure assistant - describe your business needs and I'll handle all the technical complexity",
  inputSchema: {
    type: 'object',
    properties: {
      intent: {
        type: 'string',
        description:
          'What you want to achieve (e.g., "Set up infrastructure for my e-commerce startup", "Add fraud protection to my payment API")',
      },
      context: {
        type: 'object',
...
class InfrastructureAnalyzer {
  ...
  analyzeIntent(
    ...
    if (
      lowerIntent.includes('e-commerce') ||
      lowerIntent.includes('shop') ||
      lowerIntent.includes('store')
    ) {
      ...
      lowerIntent.includes('media') ||
      lowerIntent.includes('video') ||
      lowerIntent.includes('streaming')
    ) {
      businessType = 'media';
    ...
    if (
      lowerIntent.includes('fast') ||
      lowerIntent.includes('speed') ||
      lowerIntent.includes('performance')
    ) {
    ...
    if (
      lowerIntent.includes('startup') ||
      lowerIntent.includes('new') ||
      lowerIntent.includes('small')
    ) {
  ...
  private estimateCost(context: Partial<BusinessInfrastructureContext>): string {
    if (context.scaling_expectation === 'startup') {
      return '$500-1000/month';
    }
    if (context.scaling_expectation === 'enterprise') {
      return '$5000+/month';
    }
  ...
  private estimateTime(context: Partial<BusinessInfrastructureContext>): string {
    if (context.deployment_strategy === 'immediate') {
      return '1-2 days';
    }
    if (context.deployment_strategy === 'staging_first') {
      return '1-2 weeks';
    }
    return '3-5 days';
  }
...
export async function handlePropertyInfrastructureAssistant(args: any) {
  ...
  try {
    ...
    if (
      analysis.businessContext.business_type &&
      BUSINESS_TEMPLATES[analysis.businessContext.business_type]
    ) {
      ...
      content: [
        {
          type: 'text',
          text:
            `I encountered an issue understanding your request. Let me help you differently:\n\n` +
            `Could you tell me:\n` +
      ...
      content: [
        {
          type: 'text',
          text:
            `Perfect! I'll now set up your infrastructure with a ${selection} deployment strategy.\n\n` +
