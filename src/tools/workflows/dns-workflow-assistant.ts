...
export const dnsDomainAssistant: Tool = {
  name: 'dns',
  description:
    "Maya's DNS assistant for ALECS - I make DNS changes safe and simple, with rollback plans for everything",
  inputSchema: {
    type: 'object',
    properties: {
      intent: {
        type: 'string',
        description:
          'What you need help with (e.g., "Migrate my domain from Cloudflare", "Add email records", "Fix DNS issues")',
      },
      domain: {
        type: 'string',
...
class DNSAnalyzer {
  ...
  analyzeIntent(
    ...
    if (
      ...
      lowerIntent.includes('mx') ||
      lowerIntent.includes('mail')
    ) {
      ...
      if (lowerIntent.includes('subdomain')) {
        recordTypes.push('A', 'CNAME');
      }
      if (lowerIntent.includes('www')) {
        recordTypes.push('CNAME');
      }
      if (lowerIntent.includes('api')) {
        recordTypes.push('A', 'AAAA');
      }
    } else if (
      lowerIntent.includes('fix') ||
      lowerIntent.includes('troubleshoot') ||
      lowerIntent.includes('issue')
    ) {
      ...
      lowerIntent.includes('certificate')
    ) {
      operation = 'ssl_setup';
      riskLevel = 'low';
      recordTypes = ['CAA', 'TXT'];
  ...
  analyzeMigrationSafety(
    ...
    if (context.team_expertise === 'need_guidance') {
      safetyScore -= 15;
    }
    if (context.risk_tolerance === 'zero_downtime') {
      safetyScore -= 10;
    }
...
class DNSSafetyValidator {
  ...
  async validateChanges(
    ...
    proposedChanges.forEach((change) => {
      ...
      if (
        change.type === 'MX' &&
        !proposedChanges.find((c) => c.type === 'TXT' && c.value?.includes('v=spf1'))
      ) {
      ...
      content: [
        {
          type: 'text',
          text:
            `I want to help you with your DNS needs, but I need a bit more information.\n\n` +
            `Could you tell me:\n` +
...
async function generateMigrationGuidance(
  ...
  if (
    ...
    PROVIDER_MIGRATIONS[args.context.current_provider as keyof typeof PROVIDER_MIGRATIONS]
  ) {
    const migration =
      PROVIDER_MIGRATIONS[args.context.current_provider as keyof typeof PROVIDER_MIGRATIONS];
 ...
 */
async function generateEmailSetupGuidance(args: any, analyzer: DNSAnalyzer): Promise<string> {
 ...
 */
async function generateTroubleshootingGuidance(args: any, analyzer: DNSAnalyzer): Promise<string> {
 ...
 */
async function generateGeneralDNSGuidance(args: any, analysis: any): Promise<string> {
