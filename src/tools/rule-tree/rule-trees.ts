 ...
 */
export class RuleTreeOperations extends AkamaiOperation {
      ...
      async (client) => {
        ...
        if (params.propertyId && params.version) {
          try {
            ...
            if (response.errors) {
              validationErrors.push(...(response as any).errors.map((e: any) => e.detail));
            }
            if (response.warnings) {
              warnings.push(...(response as any).warnings.map((w: any) => w.detail));
            }
          } catch (error) {
...
export const ruleTreeOperations = new RuleTreeOperations();
export const ruleTreeTools = ruleTreeOperations; // @deprecated - use ruleTreeOperations