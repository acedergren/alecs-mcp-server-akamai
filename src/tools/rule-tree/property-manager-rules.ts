...
export async function listAvailableBehaviors(
  client: AkamaiClient,
  args: z.infer<typeof RuleTreeSchemas.listBehaviors>
    ...
    for (const [category, behaviors] of Object.entries(behaviorsByCategory)) {
      ...
      for (const behavior of behaviors as any[]) {
        ...
        if (behavior.deprecated) {
          text += `- **Deprecated:** Use ${behavior.deprecatedMessage || 'alternative behavior'}\n`;
        }
        text += '\n';
      }
    }
...
export async function listAvailableCriteria(
  client: AkamaiClient,
  args: z.infer<typeof RuleTreeSchemas.listCriteria>
): Promise<MCPToolResponse> {
  try {
    // Get property details if needed
    let productId = args.productId;
    ...
    for (const [category, criteria] of Object.entries(criteriaByCategory)) {
      ...
      for (const criterion of criteria as any[]) {
        ...
        if (criterion.deprecated) {
          text += `- **Deprecated:** Use ${criterion.deprecatedMessage || 'alternative criterion'}\n`;
        }
        text += '\n';
      }
    }
...
export async function patchPropertyRules(
  client: AkamaiClient,
  args: z.infer<typeof RuleTreeSchemas.patchRules>
): Promise<MCPToolResponse> {
  try {
    ...
    if (validatedPatchResponse.errors && validatedPatchResponse.errors.length > 0) {
      ...
      for (const error of validatedPatchResponse.errors) {
        text += `- ${error.detail}\n`;
      }
      text += '\n';
    }
...
export async function bulkSearchProperties(
  client: AkamaiClient,
  args: z.infer<typeof RuleTreeSchemas.bulkSearch>
): Promise<MCPToolResponse> {
  try {
...
export async function getBulkSearchResults(
  client: AkamaiClient,
  args: z.infer<typeof RuleTreeSchemas.getBulkSearchResults>
): Promise<MCPToolResponse> {
  try {
    const response = await client.request({
...
export async function generateDomainValidationChallenges(
  ...
  args: z.infer<typeof RuleTreeSchemas.generateDomainValidation>
): Promise<MCPToolResponse> {
  try {
...
export async function resumeDomainValidation(
  _client: AkamaiClient,
  args: z.infer<typeof RuleTreeSchemas.resumeDomainValidation>
): Promise<MCPToolResponse> {
  try {
    ...
    text += '3. 🔍 Akamai will check the validation challenges\n';
...
export async function getPropertyAuditHistory(
  client: AkamaiClient,
  args: z.infer<typeof RuleTreeSchemas.getAuditHistory>
): Promise<MCPToolResponse> {
  try {
    ...
    text += '- **Hostname Changes**: Added or removed domains\n';
