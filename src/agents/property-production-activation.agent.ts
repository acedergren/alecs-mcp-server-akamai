...
export class PropertyProductionActivationAgent {
  ...
  async execute(config: ProductionActivationConfig): Promise<ProductionActivationResult> {
    ...
    try {
      ...
      console.error('[ProductionActivation] Error:', _error);
      result.errors!.push(
        `Activation _error: ${_error instanceof Error ? _error.message : String(_error)}`,
      );
      return result;
    }
  }
  ...
  private async waitForActivation(
    ...
    while (Date.now() - startTime < maxWaitMs) {
      try {
        ...
        console.error('[ProductionActivation] Status check _error:', _error);
        // Continue waiting
      }
    }
