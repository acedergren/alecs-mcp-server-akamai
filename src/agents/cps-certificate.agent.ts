...
export class CPSCertificateAgent {
  ...
  async enrollCertificate(
    ...
    try {
      ...
      const enrollmentId = parseInt(response.headers.location.split('/').pop());
          ...
          if (challenge.type === 'dns' && challenge.dnsTarget) {
            ...
            if (autoCreateRecords && this.dnsAgent) {
              ...
              try {
                ...
                recordSpinner.fail(`Failed to create DNS record: ${error.message}`);
                throw error;
              }
            }
          }
  ...
  async deployCertificate(
    ...
    try {
      ...
      const deploymentId = parseInt(response.headers.location.split('/').pop());
    ...
    await trackProgress(
      ...
      async (propertyId, index) => {
        ...
        try {
          ...
          spinner.fail(`Failed to link property ${propertyId}: ${error.message}`);
          throw error;
        }
      },
  ...
  async setupAutoRenewal(
    ...
    try {
      ...
      spinner.fail(`Failed to setup auto-renewal: ${error.message}`);
      throw error;
    }
  }
  ...
  async displayCertificateStatus(enrollmentId: number): Promise<void> {
    ...
    try {
      ...
      spinner.fail(`Failed to fetch certificate status: ${error.message}`);
      throw error;
    }
  }
  ...
  async provisionAndDeployCertificate(
    ...
    try {
      ...
      const certificate = await this.enrollCertificate(
        ...
        domains,
        { organization: options.organizationDetails }
      );
