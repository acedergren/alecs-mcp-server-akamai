import { EdgeGridAuth } from '../utils/edgegrid-auth';
import { 
  ProgressBar, 
  Spinner, 
  MultiProgress, 
...
export class CDNProvisioningAgent {
  ...
  constructor(
    ...
    this.auth = new EdgeGridAuth(this.customer);
  ...
  async createPropertyVersion(
    ...
    try {
      ...
      const versionLink = response.data.versionLink;
      const versionNumber = parseInt(versionLink.split('/').pop());
      ...
      spinner.fail(`Failed to create property version: ${error.message}`);
      throw error;
    }
  }
  ...
  async clonePropertyVersion(
    ...
    try {
      ...
      progress.update({ current: progress['current'], status: 'error', message: error.message });
      throw error;
    }
  }
  ...
  async updateRuleTree(
    ...
    try {
      ...
      spinner.fail(`Failed to update rules: ${error.message}`);
      throw error;
    }
  }
  ...
  async applyRuleTemplate(
    ...
    try {
      ...
      spinner.fail(`Failed to apply template: ${error.message}`);
      throw error;
    }
  }
  ...
  async createEdgeHostname(
    ...
    try {
      ...
      const edgeHostnameLink = response.data.edgeHostnameLink;
  ...
  async addHostname(
    ...
    try {
      ...
      spinner.fail(`Failed to add hostname: ${error.message}`);
      throw error;
    }
  }
  ...
  private async getPropertyVersion(
    ...
    return response.data.versions.items[0];
  }
  ...
  private async getLatestVersion(propertyId: string): Promise<number> {
    ...
    return response.data.versions.items[0].propertyVersion;
  }
  ...
  private async getPropertyHostnames(propertyId: string, version: number): Promise<any[]> {
    ...
    return response.data.hostnames.items;
  }
  ...
  private async waitForEdgeHostname(edgeHostnameId: string): Promise<EdgeHostname> {
    while (true) {
      ...
      const hostname = response.data.edgeHostnames.items[0];
  ...
  private async validateProperty(propertyId: string, version: number): Promise<void> {
    ...
    if (response.data.errors?.length > 0) {
      throw new Error(`Validation errors: ${JSON.stringify(response.data.errors)}`);
    }
  }
  ...
  private async getActivationStatus(
    ...
    return response.data.activations.items[0];
  }
  ...
  async provisionCompleteProperty(
    ...
    try {
      ...
      const createResponse = await this.auth.request({
        method: 'POST',
      ...
      propertyId = createResponse.data.propertyLink.split('/').pop();
