...
export class DNSMigrationAgent {
  ...
  async parseZoneFile(
    ...
    try {
      ...
      spinner.fail(`Parse failed: ${error.message}`);
      throw error;
    }
  }
  ...
  async generateMigrationInstructions(
    ...
    try {
      ...
      spinner.fail(`Failed to generate instructions: ${error.message}`);
      throw error;
    }
  }
  ...
  async createRecord(
    ...
    await this.withChangeList(zoneName, async () => {
      ...
      try {
        ...
        spinner.fail(`Failed to create record: ${error.message}`);
        throw error;
      }
    });
  }
  ...
  async updateRecord(
    ...
    await this.withChangeList(zoneName, async () => {
      ...
      try {
        ...
        spinner.fail(`Failed to update record: ${error.message}`);
        throw error;
      }
    });
  }
  ...
  async deleteRecord(
    ...
    await this.withChangeList(zoneName, async () => {
      ...
      try {
        ...
        spinner.fail(`Failed to delete record: ${error.message}`);
        throw error;
      }
    });
  }
  ...
  async listRecords(
    ...
    try {
      ...
      const records = response.data.recordsets || [];
      spinner.succeed(`Found ${records.length} records`);

      return records;
    } catch (error) {
      spinner.fail(`Failed to list records: ${error.message}`);
      throw error;
    }
  }
  ...
  private async withChangeList<T>(
    ...
    if (!changeListId) {
      ...
      const newChangeListId = response.headers.location.split('/').pop();
  ...
  private async bulkImportRecords(
    ...
    for (let i = 0; i < records.length; i++) {
      try {
        ...
        result.failed++;
        result.errors.push(`${records[i].name} ${records[i].type}: ${error.message}`);
      }
    ...
    await this.withChangeList(zoneName, async () => {
      for (const record of records) {
        try {
          ...
          result.failed++;
          result.errors.push(`${record.name} ${record.type}: ${error.message}`);
        }
      }
    });
  ...
  private async activateZone(zoneName: string): Promise<void> {
    ...
    try {
      ...
      spinner.fail(`Failed to activate zone: ${error.message}`);
      throw error;
    }
  }
}
